create
    definer = openmrs@`%` procedure CreatePatientsSMS(IN numPatients int)
BEGIN
...
INSERT INTO openmrs.visit (
    ...
    DATE_FORMAT(ADDDATE(NOW(), INTERVAL 1 DAY), '%Y-%m-%d %H:%i:%s'), -- Visit start date 1 day later than now
    NULL, -- No stop date
...
VALUES (
    @lastPersonID, -- Use the last inserted person_id
    DATE_FORMAT(ADDDATE(NOW(), INTERVAL 132 MINUTE), '%H:%i'), -- HH:MM format
...
INSERT INTO openmrs.messages_patient_template (
    actor_id, actor_type, service_query, service_query_type,
    patient_id, template_id, uuid, creator,
    changed_by, date_changed, date_created, date_voided,
    void_reason, voided, voided_by, calendar_service_query
) VALUES (
...
SET @lastMessageTemplateID := (SELECT MAX(messages_patient_template_id) FROM openmrs.messages_patient_template);

-- Inserting messages_template_field_value for the last inserted patient
INSERT INTO openmrs.messages_template_field_value (
    value, template_field_id, patient_template_id, uuid,
    creator, changed_by, date_changed, date_created,
    date_voided, void_reason, voided, voided_by
) VALUES (
    DATE(NOW()), 2, @lastMessageTemplateID, UUID(),
    1, NULL, NULL, '2023-11-17 13:19:55',
    NULL, NULL, 0, NULL
), (
    'SMS', 1, @lastMessageTemplateID, UUID(),
    1, NULL, NULL, '2023-11-17 13:19:55',
    NULL, NULL, 0, NULL
), (
